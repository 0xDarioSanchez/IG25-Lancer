...
This is a decentralized marketplace smart contract built with Rust for Arbitrum Stylus. It enables trustless deals between payers (companies/buyers) and beneficiaries (freelancers/sellers) with an integrated dispute resolution system.

### Key Features

- **Multi-Role System**: Users can register as payers, beneficiaries, or judges
- **Deal Management**: Create, accept, and finalize deals with customizable amounts and durations
- **Escrow System**: Funds are held securely in the contract until deal completion
- **Dispute Resolution**: Built-in dispute system with evidence submission and voting
- **Fee Structure**: Configurable marketplace fee on completed deals
- **Reputation System**: Track user performance as both parties and judges
- **USDC Integration**: Native USDC token support for all transactions

### Contract Architecture

The marketplace consists of three main entities:

1. **Users**: Registered participants with roles (payer/beneficiary/judge), balances, and reputation scores
2. **Deals**: Agreements between payers and beneficiaries with amount, duration, and status tracking
3. **Disputes**: Resolution mechanism for contested deals with evidence submission and protocol integration

### Core Functionality

#### User Management
- Register with one or multiple roles
- Add roles to existing accounts
- Track balance available for withdrawal
- Monitor reputation scores

#### Deal Lifecycle
1. **Creation**: Beneficiary creates deal proposal with payer, amount, and duration
2. **Negotiation**: Payer can update amount/duration before accepting
3. **Acceptance**: Payer accepts and deposits funds into escrow
4. **Completion**: Payer finalizes deal OR beneficiary requests payment after duration + 1 week
5. **Rejection**: Payer can reject deals before acceptance

#### Dispute System
- Payer can open disputes with 50 USDC fee
...
- **Decimal Precision**: 6 decimals for USDC
- **Deployment**: Arbitrum Sepolia testnet ready
- **External Dependencies**: Protocol contract for dispute resolution
